  ...
  form do |f|
    f.inputs "Album Details" do
      ...
      f.inputs "English" do
        ...
        f.input :file_en, :as => :file, :hint => f.object.file_en.present? ? link_to(f.object.file_en.file.filename, f.object.file_en.url) : content_tag(:span, "no album file")
      ...
      f.inputs "简体" do
        ...
        f.input :file_zh_cn, :as => :file, :hint => f.object.file_zh_cn.present? ? link_to(f.object.file_zh_cn.file.filename, f.object.file_zh_cn.url) : content_tag(:span, "no album file")
      ...
      f.inputs "繁体" do
        ...
        f.input :file_zh_tw, :as => :file, :hint => f.object.file_zh_tw.present? ? link_to(f.object.file_zh_tw.file.filename, f.object.file_zh_tw.url) : content_tag(:span, "no album file")
  ...
  show do
    attributes_table do
      ...
      row :file_en do |record|
        record.file_en.present? ? link_to(record.file_en.file.filename, record.file_en.url) : content_tag(:span, "no album file")
      end
      ...
      row :file_zh_cn do |record|
        record.file_zh_cn.present? ? link_to(record.file_zh_cn.file.filename, record.file_zh_cn.url) : content_tag(:span, "no album file")
      end
      bool_row :enable_zh_tw
      row :title_zh_tw
      row :cover_zh_tw do |record|
        record.cover_zh_tw.present? ? image_tag(record.cover_zh_tw.url(:thumbnail)) : content_tag(:span, "no cover image")
      end
